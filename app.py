...
from data.loader import load_titanic, get_age_group
...
PLOTLY_BASE = dict(
    ...
    legend=dict(bgcolor=rgba(C["surface"],0.95), bordercolor=rgba(C["border"],1.0), borderwidth=1),
)
...
/* Dropdown du multiselect (liste déroulante) */
...
/* ── Header barre du dashboard ── */
.dash-topbar {{
    background:{C['surface']}; border-bottom:1px solid {C['border']};
    padding:.8rem 1.5rem; margin-bottom:1.4rem;
...
.dash-topbar-title {{
    ...
    color:{C['text']}; letter-spacing:-.02em;
    display:flex; align-items:center; gap:.7rem;
}}
.dash-topbar-badge {{
    background:{C['navy']}; color:#fff; font-size:.62rem; font-weight:700;
    padding:.2rem .6rem; border-radius:20px; letter-spacing:.06em; text-transform:uppercase;
}}

/* ── KPI cards — style "dark header + light body" ── */
.kpi-grid {{
    display:grid; grid-template-columns:repeat(4,1fr); gap:1rem; margin-bottom:1.4rem;
}}
.kpi-card {{
    background:{C['surface']}; border-radius:14px; overflow:hidden;
...
.kpi-header {{
    background:{C['navy']}; padding:.6rem 1rem;
    display:flex; align-items:center; justify-content:space-between;
}}
.kpi-title {{
...
.kpi-value {{
    ...
    color:{C['text']}; line-height:1; letter-spacing:-.03em;
}}
.kpi-delta {{
...
/* ── Tabs ── */
[data-testid="stTabs"] {{
    background:{C['surface']}; border-radius:12px 12px 0 0;
...
[data-testid="stTabs"] button {{
    ...
    text-transform:uppercase; color:{C['muted']} !important;
...
[data-testid="stTabs"] button[aria-selected="true"] {{
    ...
    border-bottom:2px solid {C['mint']} !important;
    background:rgba(78,203,160,0.06) !important;
}}
[data-testid="stTabs"] button:hover {{
    color:{C['text']} !important; background:{C['bg']} !important;
}}

/* ── Section titles ── */
.section-title {{
    font-size:.72rem; font-weight:700; letter-spacing:.1em; text-transform:uppercase;
    color:{C['muted']}; margin:1rem 0 .6rem;
    display:flex; align-items:center; gap:.6rem;
}}
.section-title::after {{ content:''; flex:1; height:1px; background:{C['border']}; }}
...
.page-header-bar {{
    ...
    background:linear-gradient(180deg,{C['mint']},{C['sky']});
    flex-shrink:0;
}}
.page-header-title {{
    font-family:'Space Grotesk',sans-serif; font-size:1.1rem; font-weight:700;
    color:{C['text']}; margin:0; letter-spacing:-.02em;
}}
.page-header-sub {{ font-size:.72rem; color:{C['muted']}; margin-top:.15rem; }}

/* ── Dividers ── */
.deco-divider {{ display:flex; align-items:center; gap:1rem; margin:1.2rem 0; }}
.deco-divider::before, .deco-divider::after {{ content:''; flex:1; height:1px; background:{C['border']}; }}
.deco-divider span {{
    color:{C['muted']}; font-size:.6rem; letter-spacing:.18em;
...
.chip-navy   {{ background:rgba(26,35,50,0.08);   color:{C['navy']};  border:1px solid rgba(26,35,50,0.18); }}
.chip-mint   {{ background:rgba(78,203,160,0.1);   color:#2D9E77;      border:1px solid rgba(78,203,160,0.3); }}
.chip-sky    {{ background:rgba(91,168,245,0.1);   color:{C['sky']};   border:1px solid rgba(91,168,245,0.3); }}
.chip-coral  {{ background:rgba(240,122,101,0.1);  color:{C['coral']}; border:1px solid rgba(240,122,101,0.3); }}
.filter-count {{
    font-family:'Space Grotesk',sans-serif; font-size:1.6rem; font-weight:700; color:{C['text']};
}}
.filter-pct {{ font-size:.7rem; color:{C['muted']}; margin-left:.3rem; }}
...
.stDownloadButton > button {{
    ...
    text-transform:uppercase !important; padding:.5rem 1.4rem !important; transition:all .2s !important;
}}
.stDownloadButton > button:hover {{
    background:{C['navy2']} !important; box-shadow:0 4px 14px rgba(26,35,50,0.25) !important;
...
::-webkit-scrollbar-thumb:hover {{ background:{C['mint']}; }}
...
.sb-logo-sub {{ font-size:.6rem; color:rgba(255,255,255,0.4); letter-spacing:.12em; text-transform:uppercase; margin-top:.15rem; }}
.sb-section {{
...
.sb-footer {{
    padding:.8rem 1.2rem;
    font-size:.58rem; color:rgba(255,255,255,0.25);
...
def divider(label):
    st.markdown(f'<div class="deco-divider"><span>{label}</span></div>', unsafe_allow_html=True)
...
def kpi_card(title, value, delta_text, delta_color, icon_emoji, icon_bg):
    ...
    <div class="kpi-card">
      <div class="kpi-header">
        ...
        <span class="kpi-icon" style="background:{icon_bg}">{icon_emoji}</span>
      </div>
      <div class="kpi-body">
...
def styled_fig(fig, light=True):
    ...
    fig.update_xaxes(gridcolor=rgba(C["border"],1.0), zerolinecolor=rgba(C["border"],1.0), linecolor=rgba(C["border"],1.0))
    fig.update_yaxes(gridcolor=rgba(C["border"],1.0), zerolinecolor=rgba(C["border"],1.0), linecolor=rgba(C["border"],1.0))
    return fig
...
def render_sidebar(df):
    ...
    <div class="sb-section">
      <span class="sb-dot" style="background:{C['sky']}"></span>Classe de cabine
    </div>""", unsafe_allow_html=True)
    ...
    st.sidebar.markdown(f"""
    <div class="sb-section">
      <span class="sb-dot" style="background:{C['mint']}"></span>Genre
    </div>""", unsafe_allow_html=True)
    ...
    st.sidebar.markdown(f"""
    <div class="sb-section">
      <span class="sb-dot" style="background:{C['coral']}"></span>Tranche d'âge
    </div>""", unsafe_allow_html=True)
...
def apply_filters(df, cls, sex, ages):
    if not cls or not sex or not ages:
        return df.iloc[0:0]  # dataframe vide si rien sélectionné
    return df[df["pclass"].isin(cls) & df["sex"].isin(sex) & df["age_group"].isin(ages)]


def page_vue_generale(df, dff):
    page_header("🌊", "Vue Générale", "Synthèse · profil des passagers filtrés", "vue_generale")
    ...
    kpis_html = (
        kpi_card("Passagers", f"{total:,}",
                 f"+0 vs total", C["muted"], "👥", "rgba(91,168,245,0.2)") +
        kpi_card("Survivants", f"{survivors:,}",
                 f"sur {total:,} sélectionnés", C["mint"], "💚", "rgba(78,203,160,0.2)") +
        kpi_card("Taux de survie", f"{rate:.1f}%",
                 f"{rate:.1f}% historique", C["coral"], "📈", "rgba(240,122,101,0.2)") +
        kpi_card("Âge moyen", f"{avg_age} ans",
    ...
    with col2:
        ...
        fig2.update_traces(textinfo="percent+label",
                           marker=dict(line=dict(color="#fff",width=2)), pull=[.04,0,0])
    ...
    with col4:
        ...
        fare_df["Classe"] = fare_df["pclass"].map({1:"1ère",2:"2ème",3:"3ème"})
...
def page_analyse_survie(df, dff):
    page_header("🏅", "Analyse de Survie", "Taux · distributions · corrélations", "analyse_survie")
    ...
    fig1 = px.bar(surv_sex, x="label", y="taux", color="label",
                  ...
                  template="plotly_white", labels={"taux":"Taux (%)","label":"Genre"})
    fig1.update_traces(textposition="outside", marker_line_width=0, opacity=.9, marker_cornerradius=4)
    fig1.update_layout(showlegend=False, yaxis_range=[0,115])
    st.plotly_chart(styled_fig(fig1), use_container_width=True)

    divider("PAR CLASSE ET ÂGE")
    col1, col2 = st.columns(2)
    with col1:
        section("Survie par classe")
        sc = dff.groupby("pclass")["survived"].mean().reset_index()
        sc["taux"] = sc["survived"] * 100
        sc["Classe"] = sc["pclass"].map({1:"1ère",2:"2ème",3:"3ème"})
        fig2 = px.pie(sc, names="Classe", values="taux",
                      color_discrete_sequence=CLASS_COLORS, hole=.45, template="plotly_white")
        fig2.update_traces(textinfo="percent+label", marker=dict(line=dict(color="#fff",width=2)))
        st.plotly_chart(styled_fig(fig2), use_container_width=True)

    with col2:
        section("Distribution des âges")
        fig3 = px.histogram(dff, x="age", color="survived", nbins=30, barmode="overlay",
                            opacity=.75, color_discrete_sequence=SURVIVED_COLORS,
                            template="plotly_white",
                            labels={"age":"Âge","count":"Passagers","survived":"Survie"})
        fig3.update_layout(bargap=.02)
        st.plotly_chart(styled_fig(fig3), use_container_width=True)

    divider("HEATMAP")
    section("Taux de survie — Classe × Sexe")
    pivot = dff.groupby(["pclass","sex"])["survived"].mean().unstack() * 100
    pivot.index = pivot.index.map({1:"1ère",2:"2ème",3:"3ème"})
    ...
    divider("CORRÉLATION ÂGE · TARIF")
    section("Âge vs Tarif — coloré par survie")
    fig5 = px.scatter(dff, x="age", y="fare", color="survived",
                      color_discrete_sequence=SURVIVED_COLORS, opacity=.6,
                      template="plotly_white",
                      labels={"age":"Âge","fare":"Tarif (£)","survived":"Survie"},
...
def page_filtres(df, dff, sel_cls, sel_sex, sel_ages):
    page_header("🎛️", "Filtres Interactifs", "Explorez la sélection active", "filtres")
    log_event("filter_apply", {"classes": sel_cls, "sexes": sel_sex, "n_result": len(dff)})

    pct = len(dff) / len(df) * 100 if len(df) else 0
    cls_chips  = "".join(f'<span class="filter-chip chip-sky">Classe {c}</span>' for c in sel_cls)
    sex_chips  = "".join(f'<span class="filter-chip chip-mint">{"Homme" if s=="male" else "Femme"}</span>' for s in sel_sex)
    age_chips  = "".join(f'<span class="filter-chip chip-coral">{a}</span>' for a in sel_ages)

    st.markdown(f"""
    <div class="filter-summary">
      <div style="font-size:.6rem;font-weight:700;letter-spacing:.14em;text-transform:uppercase;
                  color:{C['muted']};margin-bottom:.7rem">Filtres actifs</div>
      <div class="filter-row">{cls_chips or '<span style="color:#ccc;font-size:.72rem">Aucune classe</span>'}</div>
      <div class="filter-row">{sex_chips or '<span style="color:#ccc;font-size:.72rem">Aucun genre</span>'}</div>
      <div class="filter-row" style="margin-bottom:.8rem">{age_chips or '<span style="color:#ccc;font-size:.72rem">Aucun âge</span>'}</div>
      <div style="border-top:1px solid {C['border']};padding-top:.7rem;display:flex;align-items:baseline;gap:.4rem">
        <span class="filter-count">{len(dff)}</span>
        <span class="filter-pct">passagers sélectionnés — {pct:.1f}% du total</span>
      </div>
    </div>""", unsafe_allow_html=True)

    col1, col2 = st.columns(2)
    with col1:
        section("Survie dans la sélection")
        ...
        fig1 = px.bar(sc, x="Statut", y="n", color="Statut",
                      color_discrete_sequence=SURVIVED_COLORS, text="n", template="plotly_white")
        fig1.update_traces(textposition="outside", marker_line_width=0, opacity=.9, marker_cornerradius=4)
        fig1.update_layout(showlegend=False)
        st.plotly_chart(styled_fig(fig1), use_container_width=True)

    with col2:
        section("Répartition par tranche d'âge")
        ag = dff["age_group"].value_counts().reset_index()
        ag.columns = ["Tranche","n"]
        fig2 = px.bar(ag, x="Tranche", y="n", color="Tranche",
                      color_discrete_sequence=[C["mint"],C["sky"],C["navy"],C["amber"],C["coral"]],
                      text="n", template="plotly_white")
        fig2.update_traces(textposition="outside", marker_line_width=0, opacity=.9, marker_cornerradius=4)
        fig2.update_layout(showlegend=False)
        st.plotly_chart(styled_fig(fig2), use_container_width=True)

    divider("TARIFS")
    section("Tarif moyen par classe · sélection active")
    fare = dff.groupby("pclass")["fare"].mean().reset_index()
    fare["Classe"] = fare["pclass"].map({1:"1ère classe",2:"2ème classe",3:"3ème classe"})
    fig3 = px.bar(fare, x="Classe", y="fare", color="Classe",
                  color_discrete_sequence=CLASS_COLORS,
                  text=fare["fare"].apply(lambda x: f"£{x:.2f}"), template="plotly_white")
    fig3.update_traces(textposition="outside", marker_line_width=0, opacity=.9, marker_cornerradius=4)
    fig3.update_layout(showlegend=False, yaxis_title="Tarif moyen (£)")
    st.plotly_chart(styled_fig(fig3), use_container_width=True)


def page_donnees(df, dff):
    page_header("📋", "Données Brutes", f"{len(dff)} passagers filtrés · export CSV", "donnees_brutes")

    all_cols = dff.columns.tolist()
    default = [c for c in ["survived","pclass","sex","age","fare","embarked","who","age_group"] if c in all_cols]
    sel_cols = st.multiselect("Colonnes à afficher", options=all_cols, default=default)
    if not sel_cols:
...
def main():
    ...
    with tab1: page_vue_generale(df, dff)
    with tab2: page_analyse_survie(df, dff)
    with tab3: page_filtres(df, dff, cls, sex, ages)
    with tab4: page_donnees(df, dff)
